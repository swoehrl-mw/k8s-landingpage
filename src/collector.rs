...
use kube::{api::ListParams, config::{KubeConfigOptions, Kubeconfig}, Api, Client, ResourceExt};
use serde::Serialize;
use tokio::sync::RwLock;
use std::{collections::{BTreeMap}, sync::Arc, time::Duration};

use crate::{config::{Config, RemoteCluster}, errors::{Error, Result}};
...
async fn run_collector_task(config: Config, info: IngressCollectionWrapper) {
    let refresh_interval = config.global.as_ref().and_then(|g| g.refresh_interval_seconds).unwrap_or(30);
    loop {
...
pub async fn collect_for_all_clusters(config: &Config) -> Result<IngressCollection> {
    ...
    if let Some(local) = config.local.as_ref() && local.enabled {
        if let Some(namespaces) = local.namespaces.as_ref() {
            let mut collected = Vec::new();
            for namespace in namespaces.iter() {
                collected.append(&mut collect_ingresses(config, client.clone(), Some(namespace)).await?);
            }
            result.push(transform_to_info("local".to_owned(), &local.description, collected));
        } else {
            result.push(transform_to_info("local".to_owned(), &local.description, collect_ingresses(config, client.clone(), None).await?));
        }
    }
...
async fn collect_from_remote(config: &Config, remote: &RemoteCluster, client: Client) -> Option<ClusterInfo> {
    ...
    if let Some(namespaces) = remote.namespaces.as_ref() {
        ...
        Some(transform_to_info(remote.name.clone(), &remote.description, collected))
    } else {
        match collect_ingresses(config, remote_client.clone(), None).await {
            Ok(specs) => 
                Some(transform_to_info(remote.name.clone(), &remote.description, specs)),
            Err(err) => {
...
async fn kubeconfig(remote: &RemoteCluster, client: Client) -> Result<Client> {
    ...
    let error_name = format!("{}/{}", remote.kubeconfig_secret.namespace, remote.kubeconfig_secret.name);

    let secret = match secret_api.get(&remote.kubeconfig_secret.name).await {
        Ok(result) => result,
        Err(err) => {
            return Err(Error::MissingKubeconfig(format!("Could not get kubeconfig secret {error_name}: {err}")));
        }
    };
    let Some(data) = secret.data.as_ref() else {
        return Err(Error::MissingKubeconfig(format!("Could not get kubeconfig secret {error_name}: No data")));
    };
    let Some(kubeconfig_data) = data.get("value") else {
        return Err(Error::MissingKubeconfig(format!("Could not get kubeconfig secret {error_name}: No data field kubeconfig")));
    };
...
async fn collect_ingresses(config: &Config, client: Client, namespace: Option<&str>) -> Result<Vec<IngressSpec>> {
    let api = if let Some(namespace) = namespace {
        Api::<Ingress>::namespaced(client, namespace)
    } else {
        Api::<Ingress>::all(client)
    };
    let only_with_annotation = config.global.as_ref().map(|g| g.only_with_annotation).unwrap_or_default();
    ...
    for ingress in object_list {
        ...
        if only_with_annotation {
            if let Some(annotations) = ingress.metadata.annotations.as_ref() {
                if annotations.get(NAME_ANNOTATION).is_none() && annotations.get(DESCRIPTION_ANNOTATION).is_none() {
        ...
        for rule in spec.rules.unwrap_or_default() {
            ...
            for path in rule.http.unwrap_or_default().paths {
                result.push(IngressSpec {
                    name: name.clone(),
                    namespace: ingress.metadata.namespace.clone().unwrap_or_else(|| "default".to_owned()),
                    host: host.clone(),
                    tls_used: true,
                    path: path.path,
...
fn transform_to_info(cluster_name: String, description: &Option<String>, input: Vec<IngressSpec>) -> ClusterInfo {
    let ingresses = input.into_iter().map(|i| {
        let url = format!("https://{}{}", i.host, i.path.unwrap_or_else(||"/".to_owned()));
        let name = i.annotations.get(NAME_ANNOTATION).unwrap_or(&i.name);
        let description = i.annotations.get(DESCRIPTION_ANNOTATION).map(|s| s.to_owned()).unwrap_or_default();
        IngressInfo { name: name.to_owned(), description, url }
    }).collect();
    ClusterInfo {
        name: cluster_name,
